...
forward's grammar, (using the <<BNF-playground>> based on <<Nearly>>):

[source,bnf]
----
/* basing the syntax on the ASCII charset */
...
/* regular words just have lowercase alphabetical characters, but strings may have other printable characters */
<word>  ::= <chars> " " | <string> " "
