...
----
IN: testing-xor
USING: math.combinatorics prettyprint kernel sequences continuations arrays ;
...
: (test-xor) ( seq -- seq ) dup [ xor ] with-datastack 2array ;
: test-xor ( seq -- seq ) [ (test-xor) ] map ;             ! <3>


possible-inputs test-xor .
----
...
<3> <<map>>            using it to test `xor` on each input
