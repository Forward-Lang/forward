...
1,0,over-supply ,0,1,over-supply ,over-supply
,===
...
We are going to compare `word A`'s output with `Word B`'s input,
by subtracting those numbers. If they `match`, ie are equal,
then we should get the number `0`.
represent 

,===
word A's output, Word B's input, results = A-B
...
.. if `results BC` is `under-supply`, then:
... `results ABC` is `over-supply` or `under-supply` or `match`. 
